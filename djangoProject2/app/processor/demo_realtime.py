...
class DemoRealtime(IO):
    ...
    def start(self):
        ...
        if self.arg.openpose is not None:
            sys.path.append('D:/pose_estimate/openpouseDemo/st-gcn/openpose/bin')
        try:
        ...
        label_name_path = 'app/resource/kinetics_skeleton/label_name.txt'
        ...
        params = dict(model_folder='app/models', model_pose='COCO',net_resolution='368x256')
        ...
        def generate():
            while(True):
                ...
                frame = cv2.resize(image, (832, 600))
                datet = str(datetime.datetime.now())
                font = cv2.FONT_HERSHEY_SIMPLEX
                frame = cv2.putText(frame, datet, (20, 15), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--video',
                            default='app/resource/media/skateboarding.mp4',
                            help='Path to video')
        ...
        parser.set_defaults(
            config='app/config/st_gcn/kinetics-skeleton/demo_realtime.yaml')
